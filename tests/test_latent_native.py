...
def test_every_json_config_parses_and_latent_run_names_are_isolated() -> None:
    ...
    assert all(cfg.backbone.in_channels == 4 and not cfg.backbone.sample_clamp for cfg in latent)
...
def _algorithm_pair(cls, tmp_path: Path):
    ...
    if cls is ReflowAlgorithm:
        ...
        torch.save(
            {"z1": torch.randn(2, 4, 16, 16), "x0": torch.randn(2, 4, 16, 16)},
            pairs,
        )
...
def test_reflow_mock_integration_and_chunk_assembly(tmp_path: Path) -> None:
    ...
    for start, end in ((0, 2), (2, 3)):
        ...
        torch.save(
            {"start": start, "end": end,
             "z1": torch.full((count, 4, 16, 16), float(start)),
             "x0": torch.full((count, 4, 16, 16), float(end))},
    ...
    assert payload["z1"].shape == payload["x0"].shape == (3, 4, 16, 16)
