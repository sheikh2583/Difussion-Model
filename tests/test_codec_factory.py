...
Tests cover:
  ...
  - Dispatch to pretrained_kl_vae with a mocked PretrainedKLVAE.from_checkpoint
...
class TestLoadCodecDispatch:
    ...
    def test_dispatches_to_pretrained_kl_vae(self):
        """Factory calls PretrainedKLVAE.from_checkpoint when codec_type matches."""
        path = _write_checkpoint({"metadata": _valid_meta(codec_type="pretrained_kl_vae")})

        sentinel = object()
        try:
            # Patch the class in its own module so the lazy import picks it up.
            with mock.patch("codec.pretrained_vae.PretrainedKLVAE") as mock_cls:
    ...
    def test_missing_diffusers_raises_import_error(self):
        """
        If diffusers is not installed, loading a pretrained_kl_vae checkpoint
        ...
        pretrained_mod.PretrainedKLVAE.from_checkpoint.side_effect = ImportError(
