...
if (Test-Path -LiteralPath $VenvPython) {
    & $VenvPython -c "import sys; raise SystemExit(sys.version_info < (3, 9))" 2>$null
...
if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
    throw "Python 3.9+ is missing and winget is unavailable. Install Python from https://www.python.org/downloads/ and rerun this script."
}
...
foreach ($Candidate in $PythonCandidates) {
    & $Candidate.FullName -c "import sys; raise SystemExit(sys.version_info < (3, 9))" 2>$null
    if ($LASTEXITCODE -eq 0) {
