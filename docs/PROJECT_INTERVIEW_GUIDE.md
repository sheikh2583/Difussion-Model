...
2. [evaluate.py](../evaluate.py#L18-L31), [train.py](../train.py#L19-L35), and the web catalog all use the shared `ALGORITHM_REGISTRY`, so their supported algorithm names stay aligned.
3. Machine labels and run-environment records document hardware provenance, but they are not cryptographic proof of where a historical run executed.
4. JSONL files are append-only. Repeated train/evaluate invocations can place multiple measurements in one metrics file; the aggregator groups them by experiment, machine, code identity, seed, record type, epoch, and NFE.
5. MF training is materially more expensive than FM because the JVP differentiates through the conditioned network, so comparing only epoch count or inference NFE is misleading.
