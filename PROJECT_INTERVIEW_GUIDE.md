...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Entry points             | [train.py](train.py), [evaluate.py](evaluate.py), [inference_server.py](inference_server.py)                                                                                                   | Start training, evaluate a checkpoint, or serve the browser UI  |
| Configuration            | [config/config.py](config/config.py), [config/fm_full.json](config/fm_full.json), [config/fm_lognorm_rtx3060.json](config/fm_lognorm_rtx3060.json), [config/mf_full.json](config/mf_full.json) | Shared controls and algorithm-only options                      |
...
`results/fm_lognorm_rtx3060` contains a complete 100-epoch `FlowMatchingLognormAlgorithm` run with checkpoints at ten-epoch intervals. Its configuration uses batch size 32, learning rate $2\times10^{-4}$, and the same backbone. The records show lower training loss than the supplied FM run and evaluation values at NFE 1, 5, 10, and 20. `generate_checkpoints_3060.py` explicitly loads this run and writes visual grids under `results/checkpoint_samples/fm_lognorm`.
